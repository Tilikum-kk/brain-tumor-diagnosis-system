...
class HGMFNet(nn.Module):
    ...
    def forward(self, mri_images: torch.Tensor,
        ...
        attn_out, attn_weights = self.cross_modal_fusion(all_nodes, all_nodes, all_nodes)
        # attn_out: (B, 257, H_dim), attn_weights: (B, 257, 257)

        # 从注意力矩阵中提取各模态贡献度
        # all_nodes = [模态0×64 | 模态1×64 | 模态2×64 | 模态3×64 | 临床×1] = 257个节点
        node_per_mod = pooled_size ** 3  # 64
        modality_attn = []
        for m in range(self.in_channels):
            start = m * node_per_mod
            end = start + node_per_mod
            # 该模态所有节点作为key被关注的总权重
            mod_weight = attn_weights[:, :, start:end].mean(dim=(1, 2))  # (B,)
            modality_attn.append(mod_weight)
        real_attention = torch.stack(modality_attn, dim=1)  # (B, 4)
        real_attention = real_attention / (real_attention.sum(dim=1, keepdim=True) + 1e-8)  # 归一化
        # 扩展为与旧接口兼容的形状 (B, 4, N)
        attention_weights = real_attention.unsqueeze(-1).expand(-1, -1, pooled_size**3)
