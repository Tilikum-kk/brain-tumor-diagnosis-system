...
class ValidationVisualizer:
    ...
    def save_multi_modal_overview(
        ...
        # 选肿瘤面积最大的切片（而非固定中间层）
        if prediction.ndim == 3:
            tumor_per_slice = (prediction > 0).sum(axis=(1, 2))
            best_z = int(np.argmax(tumor_per_slice)) if tumor_per_slice.max() > 0 else prediction.shape[0] // 2
        else:
            best_z = prediction.shape[0] // 2 if prediction.ndim == 3 else 0
