...
class ExaminationResponse(BaseModel):
    ...
    exam_date: datetime
    status: str
    tumor_volume_ml: Optional[float]
    edema_volume_ml: Optional[float]
    malignant_probability: Optional[float]
    predicted_who_grade: Optional[int]
...
async def generate_report(
    ...
    with db_manager.get_session() as session:
        ...
        report_summary = (
            ...
            f"{'男' if str(patient.gender) == 'male' else '女'}，"
        ...
        # 生成PDF报告
        report_gen = get_report_generator()
        pdf_path = REPORT_DIR / f"report_exam_{exam_id}_{uuid.uuid4().hex[:8]}.pdf"
        ...
        try:
            report_gen.generate_pdf_report(
                ...
                patient_info={
                    ...
                    'age': patient.age,
                    'gender': '男' if str(patient.gender) == 'male' else '女',
...
async def download_report(
    ...
    with db_manager.get_session() as session:
        ...
        return FileResponse(
            ...
            filename=f"脑肿瘤诊断报告_检查{exam_id}.pdf",
