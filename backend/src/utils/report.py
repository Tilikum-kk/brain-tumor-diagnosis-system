...
class ReportGenerator:
    ...
    def save_segmentation_overlay(self, mri_slice: np.ndarray,
        ...
        if mri_slice.ndim == 3:
            if slice_idx is None:
                slice_idx = mri_slice.shape[0] // 2  # 取中间切片
        ...
        axes[0].set_title('原始MRI图像', fontsize=12)
        axes[0].axis('off')
        ...
        axes[1].imshow(masked, cmap=cmap, alpha=0.6, vmin=0, vmax=3)
        axes[1].set_title('肿瘤分割掩码', fontsize=12)
        axes[1].axis('off')
        ...
        for label_val, color, name in [
            ...
            mask_label = (seg_mask == label_val).astype(np.uint8)
            if mask_label.sum() > 0:
                contours = measure.find_contours(mask_label, 0.5)
                for contour in contours:
                    axes[2].plot(contour[:, 1], contour[:, 0],
                                color=color, linewidth=1.5, label=name)
        axes[2].legend(fontsize=8, loc='upper right')
        axes[2].set_title('分割覆盖图', fontsize=12)
        axes[2].axis('off')
    ...
    def generate_pdf_report(self, output_path: str,
        ...
        info_data = [
            ...
            ['检查日期', examination_info.get('exam_date', report_date)],
            ['报告日期', report_date],
