      ...
      <el-col :span="8">
        <div class="card patient-card">
          <div class="card-header patient-header">
            ...
            <div>
              <h3>{{ selectedPatientId ? '已选择患者' : '患者信息' }}</h3>
              <p>{{ selectedPatientId ? patientLabel : '搜索已有患者或快速创建' }}</p>
            </div>
            <el-tag v-if="selectedPatientId" type="success" size="small" effect="light">已关联</el-tag>
          </div>

          <!-- 搜索已有患者 -->
          <div class="section-block search-block">
          ...
          <div class="or-divider">
            ...
            <span class="or-text">或 快速建档</span>
            <span class="or-line"></span>
          </div>

          <!-- 快速创建 — 紧凑布局 -->
          <div class="section-block create-block">
            <div class="field-row">
              <div class="field-item flex-1">
                <label class="field-label">患者编码 <span class="required">*</span></label>
            ...
            <div class="field-row three-col">
              <div class="field-item">
                ...
                <el-input-number v-model="quick.age" :min="0" :max="120" placeholder="岁" size="default" style="width:100%" controls-position="right" />
              </div>
              <div class="field-item">
                <label class="field-label">性别</label>
                <el-radio-group v-model="quick.gender" size="small" class="gender-group">
              ...
              <div class="field-item">
                ...
                <el-select v-model="quick.grade" placeholder="分级" size="default" style="width:100%">
                  <el-option label="未知" :value="null" />
                  <el-option label="I 级" :value="0" />
                  <el-option label="II 级" :value="1" />
                  <el-option label="III 级" :value="2" />
                  <el-option label="IV 级" :value="3" />
                </el-select>
              </div>
            </div>
            ...
            <div class="field-row">
              <div class="field-item flex-1">
                <label class="field-label">肿瘤位置</label>
                <el-select v-model="quick.location" placeholder="选择脑区" size="default" style="width:100%">
            ...
            <div class="kps-block">
              <label class="field-label">
                ...
                <el-tooltip placement="top" effect="dark">
                  <template #content>
                    <div style="max-width:260px;line-height:1.6;font-size:12px">
                      <p style="margin:0 0 4px"><b>KPS 评分</b> — 评估患者日常活动能力</p>
                      <p style="margin:1px 0">100 正常 · 90 轻微症状 · 80 勉强活动 · 70 自理</p>
                      <p style="margin:1px 0">60 偶尔需帮助 · 50 常需帮助 · 40 不能自理</p>
                      <p style="margin:1px 0">30 严重不自理 · 20 病重 · 10 病危 · 0 死亡</p>
                    </div>
                  </template>
              ...
              <div class="kps-row">
                <el-slider v-model="quick.kps" :min="0" :max="100" :step="10" :marks="{0:'0',50:'50',100:'100'}" />
                <span class="kps-value" :class="quick.kps >= 80 ? '' : quick.kps >= 50 ? 'mid' : 'low'">{{ quick.kps }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="action-area">
            ...
            <div v-if="!canAnalyze && !analyzing" class="hint-text">
              ...
              <span>请选择或创建患者，并上传至少一种 MRI 影像</span>
            </div>
          </div>
        </div>
      </el-col>
...
import {
  ...
  SuccessFilled, View, DocumentAdd, RefreshLeft, Search, FolderOpened, QuestionFilled,
...
const patientLabel = computed(() => {
  const p = patientOptions.value.find(x => x.id === selectedPatientId.value)
  if (!p) return ''
  return `${p.patient_code} · ${p.full_name || '未填写'}`
})
...
/* ===== 患者表单（优化版）===== */
.patient-card { padding-bottom: 20px; }
.patient-header { gap: 12px; }
.patient-avatar {
  width: 40px; height: 40px; border-radius: 10px; background: linear-gradient(135deg, #e6f4ff, #d6e4ff);
  display: flex; align-items: center; justify-content: center; color: #1677ff; flex-shrink: 0;
}
.section-block { padding: 14px 20px; }
.search-block { padding-bottom: 8px; }
.create-block { padding-top: 4px; }
.section-label {
  display: flex; align-items: center; gap: 6px;
  font-size: 12px; color: #999; margin-bottom: 8px; letter-spacing: 0.5px;
}
.or-divider {
  display: flex; align-items: center; gap: 10px; padding: 0 20px; margin: 4px 0 8px;
}
.or-line { flex: 1; height: 1px; background: #f0f0f0; }
.or-text { font-size: 11px; color: #ccc; white-space: nowrap; }

.field-row { display: flex; gap: 10px; margin-bottom: 8px; }
.field-row.three-col .field-item { flex: 1; }
.field-item.flex-1 { flex: 1; }
.field-label { font-size: 12px; color: #888; margin-bottom: 3px; display: block; font-weight: 500; }
.field-label .required { color: #ff4d4f; }
.full-width { width: 100%; }
.gender-group { display: flex; width: 100%; }
.gender-group .el-radio-button { flex: 1; }
.gender-group .el-radio-button__inner { width: 100%; text-align: center; }
.help-icon { color: #bbb; cursor: help; vertical-align: -2px; }
.help-icon:hover { color: #1677ff; }

.kps-block { margin-top: 4px; padding: 10px 12px; background: #fafbfc; border-radius: 8px; border: 1px solid #f0f0f0; }
.kps-row { display: flex; align-items: center; gap: 12px; }
.kps-row .el-slider { flex: 1; }
.kps-value {
  width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center;
  font-size: 15px; font-weight: 700; flex-shrink: 0;
}
.kps-value { background: #f6ffed; color: #52c41a; }
.kps-value.mid { background: #fffbe6; color: #faad14; }
.kps-value.low { background: #fff1f0; color: #ff4d4f; }

.action-area { padding: 12px 20px 4px; }
